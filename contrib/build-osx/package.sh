...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-Noir.app"
    exit -127
fi
...
dmg dmg Electrum_uncompressed.dmg electrum-noir-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_uncompressed.dmg

echo "Done."
md5sum electrum-noir-$VERSION.dmg
