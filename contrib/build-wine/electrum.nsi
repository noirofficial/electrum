...
;Variables
  ...
  !define PRODUCT_NAME "Noir Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/noirofficial/electrum"
  !define PRODUCT_PUBLISHER "The Noir Team"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/noir-electrum-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\noir-electrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\noir-electrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\noir-electrum-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\noir-electrum-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\noir-electrum-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\noir-electrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
