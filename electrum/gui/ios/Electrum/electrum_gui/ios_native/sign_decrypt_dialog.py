...
class SignDecryptVC(SignDecryptBase):
    ...
    def initWithMode_(self, mode: int) -> ObjCInstance:
        ...
        if mode not in (SignVerify, EncryptDecrypt):
            utils.NSLog(
                " *** ERROR -- mode %d passed to SignDecryptVC.initWithMode is not valid! Defaulting to mode 'SignVerify'",
                mode)
            mode = 0
    ...
    def loadView(self) -> None:
        ...
        spacer = UIBarButtonItem.alloc().initWithBarButtonSystemItem_target_action_(UIBarButtonSystemItemFlexibleSpace,
                                                                                    None, None).autorelease()
        item = UIBarButtonItem.alloc().initWithBarButtonSystemItem_target_action_(UIBarButtonSystemItemDone, self, SEL(
            b'onCloseKeyboard:')).autorelease()
    ...
    def refresh(self) -> None:
        ...
        if data.pubkey and not isinstance(data.pubkey, str):
            data = utils.set_namedtuple_field(data, 'pubkey', data.pubkey)
            utils.nspy_put_byname(self, data, 'data')
        tf.text = str(data.address if data.address else "") if mode == SignVerify else (
            str(data.pubkey) if data.pubkey else "")
    ...
    def doSign(self) -> None:
        ...
        def onPw(password: str) -> None:
            ...
            parent().show_message(
                _("The signature for the provided message has been pasted into the signature text box."),
                title=_("Success"))
        ...
        def onPw(password: str) -> None:
            try:
                ...
                if "Incorrect password" in err:
                    err = _(
                        "The specified public key cannot decrypt this cyphertext.\nPlease specify the correct key to decrypt.")
