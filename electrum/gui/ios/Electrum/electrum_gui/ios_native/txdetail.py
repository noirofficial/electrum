...
class TxInputsOutputsTVC(TxInputsOutputsTVCBase):
    ...
    @objc_method
    def initWithInputTV_outputTV_timestamp_(self, inputTV: ObjCInstance, outputTV: ObjCInstance,
        ...
        if hdr:
            ...
            try:
                if tv.tag == self.tagin:
                    lbl.text = _("Inputs") + (" (%d) " % len(tx.inputs()))
                elif tv.tag == self.tagout:
                    lbl.text = _("Outputs") + (" (%d) " % len(tx.outputs()))
            except:
    ...
    def tableView_cellForRowAtIndexPath_(self, tv, indexPath):
        ...
        try:
            ...
            if tv.tag == self.tagin:
                ...
                raise ValueError("tv tag %d is neither input (%d) or output (%d) tag!" % (
            ...
            if isInput:
                if x['type'] == 'coinbase':
                    ...
                    if x.get('value') is not None:
                        ...
                        if fx(): mytxt += ' (' + fx().historical_value_str(v_in, timestamp_to_datetime(
                ...
                if v is not None:
                    cell.detail.text = (format_amount(v) + " " + base_unit + ((' (' + fx().historical_value_str(v,
                                                                                                                timestamp_to_datetime(
                                                                                                                    self.ts)) + " " + fx().get_currency() + ')') if fx() else '')).strip()
    ...
    def tableView_didSelectRowAtIndexPath_(self, tv, indexPath):
        ...
        vc = self.txDetailVC
        title = _("Options")
        message = _("Transaction Input {}").format(indexPath.row) if isInput else _("Transaction Output {}").format(
            indexPath.row)
            ...
            if parent.wallet.is_mine(addy):
                ...
                if not entry:  # is not mine, isn't in contacts, offer user the option of adding
                    def doAddNewContact(addy):
                        contacts.show_new_edit_contact(addy, self.txDetailVC,
                                                       onEdit=lambda x: utils.show_notification(_("Contact added")),
                                                       title=_("New Contact"))
                    ...
                    actions.insert(1, [_("Show Contact"), contacts.PushNewContactDetailVC, entry,
                                       self.txDetailVC.navigationController])
...
def CreateTxInputsOutputsTVC(txDetailVC: ObjCInstance, tx: Transaction, itv: ObjCInstance, otv: ObjCInstance,
...
def _setup_transaction_detail_view(vc: ObjCInstance) -> None:
    ...
    can_sign = not tx.is_complete() and wallet and wallet.can_sign(
        tx)  # and (wallet.can_sign(tx) # or bool(self.main_window.tx_external_keypairs))
    ...
    tx_hash_str = tx_hash if tx_hash is not None and tx_hash != "None" and tx_hash != "Unknown" and tx_hash != _(
        "Unknown") else _('Unknown')
    rbbs = []
    ...
    if tx_hash_str == _("Unknown") or tx_hash is None:  # unsigned tx
        ...
        vc.noTxHashLbl.text = _(
            "You need to sign this transaction in order for it to get a transaction ID.") if can_sign else _(
            "This transaction is not signed and thus lacks a transaction ID.")
        vc.notsigned = True
        rbbs.append(UIBarButtonItem.alloc().initWithImage_style_target_action_(UIImage.imageNamed_("barbut_actions"),
                                                                               UIBarButtonItemStyleBordered, vc,
                                                                               SEL(b'onShareSave:')).autorelease())
    else:
    ...
    if myAmtStr:
        ...
        ats = NSMutableAttributedString.alloc().initWithString_attributes_(am, {
            NSFontAttributeName: UIFont.systemFontOfSize_weight_(16.0, UIFontWeightBold)}).autorelease()
        if unt:
            ats.appendAttributedString_(NSAttributedString.alloc().initWithString_attributes_(unt, {
                NSFontAttributeName: UIFont.systemFontOfSize_weight_(16.0, UIFontWeightBold)}).autorelease())
        if rest:
            ats.appendAttributedString_(NSAttributedString.alloc().initWithString_attributes_(rest, {
                NSFontAttributeName: UIFont.systemFontOfSize_weight_(14.0, UIFontWeightRegular)}).autorelease())
    ...
    vc.inputsTVHeightCS.constant = min(_TxInputsOutputsHeaderHeight + _TxInputsOutputsCellHeight * len(tx.inputs()),
                                       vc.maxTVHeight)
    vc.outputsTVHeightCS.constant = min(_TxInputsOutputsHeaderHeight + _TxInputsOutputsCellHeight * len(tx.outputs()),
                                        vc.maxTVHeight)
...
class TxDetail(TxDetailBase):
    ...
    def onCpyBut_(self, but) -> None:
        entry = utils.nspy_get_byname(self, 'tx_entry')
        utils.boilerplate.vc_highlight_button_then_do(self, but,
                                                      lambda: gui.ElectrumGui.gui.copy_to_clipboard(entry.tx_hash))
    ...
    def onShareSave_(self, sender: ObjCInstance) -> None:
        parent = gui.ElectrumGui.gui
        ipadAnchor = sender.view.frame if isinstance(sender,
                                                     UIGestureRecognizer) else sender  # else clause means it's a UIBarButtonItem
    ...
    def onBroadcast(self) -> None:
        ...
        def broadcastDone():
            ...
            if status is not None and status >= 0 and status < len(StatusImages):
                ...
                utils.nspy_put_byname(self, entry, 'tx_entry')
            _setup_transaction_detail_view(
                self)  # nb: don't call refresh here, instead call this to re-evaluate everything, including 'canRefresh'

        parent.broadcast_transaction(tx, self.descTf.text, broadcastDone)


def CreateTxDetailWithEntry(entry: HistoryEntry, on_label=None, on_appear=None, tx=None,
...
def CreateTxDetailWithTx(tx: Transaction, on_label=None, on_appear=None, asModalNav=False) -> ObjCInstance:
    ...
    entry = HistoryEntry(tx, tx_hash, status_str, label_, parent.format_amount(amount) if amount is not None else _(
