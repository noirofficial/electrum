...
from .amountedit import BTCAmountEdit, FiatAmountEdit, \
    BTCkBEdit  # Makes sure ObjC classes are imported into ObjC runtime
from electrum import WalletStorage, Wallet
from electrum.util import timestamp_to_datetime, format_time, create_URI
from electrum.i18n import _, language
from electrum.paymentrequest import PR_UNPAID, PR_EXPIRED, PR_UNKNOWN, PR_PAID
...
ReqItem = namedtuple("ReqItem",
                     "dateStr addrStr signedBy message amountStr statusStr addr iconSign iconStatus fiatStr timestamp expiration expirationStr amount fiat")
...
class ReceiveVC(ReceiveBase):
    ...
    def viewDidLoad(self) -> None:
        ...
        if pr:
            ...
            if pr.addr:
                ...
                self.setReceiveAddress_(pr.addr)
            else:
    ...
    def setEditable_(self, b: bool) -> None:
        if not b:
            ...
            bb = UIBarButtonItem.alloc().initWithImage_style_target_action_(UIImage.imageNamed_('barbut_chk'),
                                                                            UIBarButtonItemStylePlain, self,
                                                                            SEL('onSaveDone:')).autorelease()
    ...
    def refresh(self) -> None:
        ...
        if self.addrStr:
            ...
            if address is not None:
                self.setReceiveAddress_(address)
    ...
    def onCopyBut_(self, sender) -> None:
        utils.boilerplate.vc_highlight_button_then_do(self, sender,
                                                      lambda: parent().copy_to_clipboard(self.addrStr, 'Address'))
    ...
    def generatePrURI(self) -> ObjCInstance:
        ...
        uri = create_URI(self.addrStr, amount, message)
        print("uri = ", uri)
        return uri
    ...
    def getUnusedAddress(self) -> None:
        ...
        if self.addrStr:
            ...
            current_address = self.addrStr
            addr = wallet.get_unused_address()
            if not current_address in domain and addr:
                self.setReceiveAddress_(addr)
    ...
    def onShareRequestBut_(self, sender) -> None:
        ...
        def OnShareQR() -> None:
            ...
            if not self.qr.image:
                parent().show_error(vc=self, message="Error Generating QR Image")
            else:
                utils.show_share_actions(vc=self, img=self.qr.image, ipadAnchor=ipadAnchor, objectName=_("Image"))
    ...
    def onQRImgTap(self) -> None:
        if not self.qr.image:
            parent().show_error(vc=self, message="Error, No QR Image")
        else:
            def ShowIt() -> None:
...
class RequestsMgr(utils.DataMgr):
    def doReloadForKey(self, ignored):
        ...
        for req in wallet.get_sorted_requests(parent().config):
            ...
            item = ReqItem(date, address, signedBy, message, amount_str, pr_tooltips.get(status, ''), address, iconSign,
                           iconStatus, fiatStr, timestamp, expiration, expirationStr, amount, fiat)
...
class ReqTVD(ReqTVDBase):
    ...
    def tableView_cellForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        try:
            ...
            if item.fiatStr:
                cell.amount.attributedText = utils.hackyFiatAmtAttrStr(item.amountStr.strip(), item.fiatStr.strip(), '',
                                                                       2.5, cell.amountTit.textColor)
            else:
    ...
    def tableView_didSelectRowAtIndexPath_(self, tv, indexPath) -> None:
        ...
        if not self.vc or not self.vc.navigationController:
            utils.NSLog(
                "Cannot preset ReceiveVC because current view controller ReqTVD is embedded in is either undefined or lacks a navigationController!")
            return
        reqs = _GetReqs()
        try:
            item = reqs[indexPath.row]
    ...
    @objc_method
    def tableView_commitEditingStyle_forRowAtIndexPath_(self, tv: ObjCInstance, es: int,
                                                        indexPath: ObjCInstance) -> None:
    ...
    def tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        ...
        try:
            ...
            utils.NSLog("ReqTV.tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_, got exception: %s",
                        str(sys.exc_info()[1]))
            traceback.print_exc(file=sys.stderr)
        return None
...
class ReqTVDTiny(ReqTVD):
    ...
    def tableView_cellForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        ...
        cell.detailTextLabel.text = ((item.addrStr + " ") if item.addrStr else "") + (
            item.amountStr if item.amountStr else "") + " - " + item.statusStr
