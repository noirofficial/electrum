...
def uicolor_custom(name: str) -> ObjCInstance:
    ...
    if not _ColorScheme:
        ...
        _ColorScheme = {
            ...
            'nav': UIColor.colorInDeviceRGBWithHexString_("#2B001E").retain(),
            'link': UIColor.colorInDeviceRGBWithHexString_("#2B001E").retain(),
    ...
    if name in ['blue', 'myblue', 'tf', 'password']:
        return UIColor.colorWithRed_green_blue_alpha_(0.91746425629999995, 0.95870447160000005, 0.99979293349999998,
                                                      1.0)
...
def uilabel_replace_attributed_text(lbl: ObjCInstance, text: str, template: ObjCInstance = None,
                                    font: ObjCInstance = None) -> ObjCInstance:
...
def show_share_actions(vc: ObjCInstance,
                       ...
                       # optional completion function that gets called when alert is presented
                       ipadAnchor: object = None,
                       animated: bool = True,
                       finishedCompletion: Callable[[], str] = None,
                       # optional completion function that gets called when alert is finished. the string passed is the UIActivityType the user selected, or None if the user cancelled the activity
                       objectName: str = None
                       # the descriptive name of the object eg 'File' or 'Transaction' or 'Wallet', translated
                       ) -> ObjCInstance:
...
### Show modal alert
###################################################
def show_please_wait(vc: ObjCInstance, message: str, animated: bool = True,
                     completion: Callable[[], None] = None) -> ObjCInstance:
    pw = None
    try:
    ...
    if not pw:
        return show_alert(vc, title=_("Please wait"), message=message, actions=[], animated=animated,
...
def show_alert(vc: ObjCInstance,  # the viewcontroller to present the alert view in
               ...
               localRunLoop: bool = False,
               # whether or not to create a local event loop and block until dialog finishes.. useful for full stop error messages and/or password dialogs
               uiTextFieldHandlers: list = None,
               # if you want to create custom UITextFields in this alert, and the alert'ss type is UIAlertControllerStyleAlert, pass a list of fully annotated callbacks taking an objc_id as arg and returning None, one for each desired text fields you want to create
               ipadAnchor: object = None
               # A CGRect -- use this on ipad to specify an anchor if using UIAlertControllerStyleActionSheet
               ) -> ObjCInstance:
    if localRunLoop:
        NSLog(
    ...
    if uiTextFieldHandlers:
        ...
        for h in uiTextFieldHandlers:
            alert.addTextFieldWithConfigurationHandler_(Block(
                h))  # let's hope h is a callable of the right type with the right number of args else exception will be thrown here
...
def show_tf_alert(vc: ObjCInstance, title: str, message: str,
                  ...
                  spellCheckingType=UITextSpellCheckingTypeNo) -> ObjCInstance:
    tf = None

    def SetupTF(tfo: objc_id) -> None:
        nonlocal tf
        tf = ObjCInstance(
            tfo).retain()  # need to retain it because it will get released when dialog goes away, but we want its data in MyOnOk below..
...
def present_modal_picker(parentVC: ObjCInstance,
    ...
    parentVC.view.endEditing_(
        True)  # NB: do not use setDisablesAutomaticKeyboardDismissal because it is missing on newer iOS! (caused an app crash) -- so we do this instead
...
def show_notification(message: str,
                      duration: float = 2.0,
                      # the duration is in seconds may be None but in that case must specify a completion
                      ...
                      onTapCallback: Callable[[], None] = None,
                      # the function to call if user taps notification -- should return None and take no args
    ...
    if isinstance(color, UIColor):
        pass
    elif color is None or not isinstance(color, (tuple, list)) or len(color) != 4 or [c for c in color if
                                                                                      type(c) not in [float, int]]:
        color = uicolor_custom('notif')
    else:
        color = UIColor.colorWithRed_green_blue_alpha_(*color)
    if isinstance(textColor, UIColor):
        pass
    elif textColor is None or not isinstance(textColor, (tuple, list)) or len(textColor) != 4 or [c for c in textColor
                                                                                                  if
                                                                                                  type(c) not in [float,
                                                                                                                  int]]:
...
def makeFancyDateAttrString(datestr: str, font: ObjCInstance = None) -> ObjCInstance:
    ...
    if datestr: datestr = datestr.translate(
        {ord('-'): '.'})  # replace hyphens in date with '.' chars as per Max's recommendations
...
def hackyFiatAmtAttrStr(amtStr: str, fiatStr: str, ccy: str, pad: float, color: ObjCInstance = None,
                        cb: Callable = None, kern: float = None, amtColor=None, isIpad=False) -> ObjCInstance:
...
def register_keyboard_callbacks(view: ObjCInstance, onWillHide=None, onWillShow=None, onDidHide=None,
                                onDidShow=None) -> int:
    if not any([onWillHide, onWillShow, onDidShow, onDidShow]) or not view or not isinstance(view, UIView):
        NSLog(
            "WARNING: register_keyboard_callbacks: need at least one callback specified, as well as non-null view! Will return early!")
        return 0
    global _kbcb_idx
    _kbcb_idx += 1
    handle = _kbcb_idx
    ...
    if entry.onWillHide: NSNotificationCenter.defaultCenter.addObserver_selector_name_object_(entry.handler,
                                                                                              SEL('willHide:'),
                                                                                              UIKeyboardWillHideNotification,
                                                                                              None)
    if entry.onWillShow: NSNotificationCenter.defaultCenter.addObserver_selector_name_object_(entry.handler,
                                                                                              SEL('willShow:'),
                                                                                              UIKeyboardWillShowNotification,
                                                                                              None)
    if entry.onDidHide: NSNotificationCenter.defaultCenter.addObserver_selector_name_object_(entry.handler,
                                                                                             SEL('didHide:'),
                                                                                             UIKeyboardDidHideNotification,
                                                                                             None)
    if entry.onDidShow: NSNotificationCenter.defaultCenter.addObserver_selector_name_object_(entry.handler,
                                                                                             SEL('didShow:'),
...
def unregister_keyboard_callbacks(handle: int) -> None:
    ...
    if entry:
        if entry.onWillHide: NSNotificationCenter.defaultCenter.removeObserver_name_object_(entry.handler,
                                                                                            UIKeyboardWillHideNotification,
                                                                                            None)
        if entry.onWillShow: NSNotificationCenter.defaultCenter.removeObserver_name_object_(entry.handler,
                                                                                            UIKeyboardWillShowNotification,
                                                                                            None)
        if entry.onDidHide: NSNotificationCenter.defaultCenter.removeObserver_name_object_(entry.handler,
                                                                                           UIKeyboardDidHideNotification,
                                                                                           None)
        if entry.onDidShow: NSNotificationCenter.defaultCenter.removeObserver_name_object_(entry.handler,
                                                                                           UIKeyboardDidShowNotification,
                                                                                           None)
    ...
    def kbShow(r: CGRect) -> None:
        ...
        if resp and isinstance(resp, UIView) and window and resp.window():
            ...
            if diff:
                ...
                '''
                scrollPoint = CGPoint(0.0,
                                      sv.contentOffset.y + diff)  # origin.y - visible.size.height + respFrame.size.height + 10)
...
class FileBackedDict(object):
    def __init__(self, fileName: str, other: object = None):
        ...
        if isinstance(other, FileBackedDict):
            ...
            if self.write():
                NSLog("File-backed dict '%s' created as copy of '%s'", os.path.split(self._fn)[-1],
                      os.path.split(other._fn)[-1])
        else:
            if os.path.exists(self._fn):
                self.read()
            else:
                NSLog("New empty file-backed dict '%s' -- will create file once data is added.",
                      os.path.split(self._fn)[-1])
    ...
    def pop(self, key: Any, save: bool = True) -> Any:
        if not isinstance(save, bool):
            NSLog(
                "*** WARNING: FileBackedDict's pop() method doesn't take a default value. The second argument is always the 'save' arg!")
...
class SecureKeyEnclave:
    ...
    def __init__(self, keyDomain: str):
        self._keyInterface = KeyInterface.keyInterfaceWithPublicKeyName_privateKeyName_(keyDomain + ".pubkey",
                                                                                        keyDomain + ".privkey").retain()
    ...
    def decrypt_data(self, data: bytes, completion: Callable[[bytes, str], None], prompt: str = None) -> None:
        ...
        if not isinstance(data, bytes): raise ValueError(
            'A bytes or str object is required as the first argument to this function!')
