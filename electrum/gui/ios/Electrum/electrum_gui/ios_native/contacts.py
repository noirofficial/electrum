...
#
from electrum.bitcoin import is_address, pubkey_to_address
from . import utils
from . import gui
...
class ContactsVC(ContactsVCBase):
    ...
    def commonInitWithMode_(self, mode: int) -> None:
        ...
        if self.mode == ModePicker:
            lbuts = [
                UIBarButtonItem.alloc().initWithBarButtonSystemItem_target_action_(UIBarButtonSystemItemCancel, self,
                                                                                   SEL(
                                                                                       b'onPickerCancel')).autorelease(),
            ]
            buts = [
                UIBarButtonItem.alloc().initWithImage_style_target_action_(UIImage.imageNamed_("barbut_chk"),
                                                                           UIBarButtonItemStyleDone, self,
                                                                           SEL(b'onPickerPayTo')).autorelease(),
                UIBarButtonItem.alloc().initWithImage_style_target_action_(UIImage.imageNamed_("barbut_plus"),
                                                                           UIBarButtonItemStylePlain, self,
                                                                           SEL(b'onAddBut')).autorelease(),
            ]
    ...
    def loadView(self) -> None:
        ...
        ats = NSMutableAttributedString.alloc().initWithAttributedString_(
            self.noContactsLabel.attributedText).autorelease()
    ...
    def tableView_didSelectRowAtIndexPath_(self, tv, indexPath):
        ...
        if self.mode == ModePicker:
            if not self.pickSingle:
                ...
                wasSel = self.setupAccessoryForCell_atIndex_(cell,
                                                             indexPath.row)  # this sometimes fails if address is frozen and/or we are watching only
                self.setIndex_selected_(indexPath.row, wasSel)

                self.selected = self.updateSelectionButtons()
            else:
                # force only 1 contact to be selected, clearing all others
                self.selected = [contacts[indexPath.row].address_str] if not self.isIndexSelected_(
                    indexPath.row) else []
                self.selected = self.updateSelectionButtons()
    ...
    def tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        ...
        try:
            ...
            utils.NSLog("ContactsTV.tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_, got exception: %s",
                        str(sys.exc_info()[1]))
            traceback.print_exc(file=sys.stderr)
        return None
    ...
    def setIndex_selected_(self, index: int, b: bool) -> None:
        try:
            ...
            sels = set(list(self.selected))
            if not b:
                sels.discard(entry.address_str)
            else:
                sels.add(entry.address_str)
    ...
    def onTapAddress_(self, view: ObjCInstance) -> None:
        ...
        show_contact_options_actionsheet(contact, self, view,
                                         onEdit=lambda x: utils.show_notification(_("Contact saved")))
    ...
    def setupAccessoryForCell_atIndex_(self, cell, index: int) -> bool:
        ...
        if no_good or not self.isIndexSelected_(index) or self.mode == ModeNormal:
            cell.customAccessory.image = UIImage.imageNamed_(
                "circle2" if self.mode == ModePicker else "chevron_gray_right")
        else:
...
class NewContactVC(NewContactBase):
    ...
    def onOk(self) -> None:
        ...
        def doCB() -> None:
            ...
            if callable(cb):
                entry = None
                if name and address_str and is_address(address_str):
                    address = address_str
...
class ContactDetailVC(ContactDetailVCBase):
    ...
    def viewDidLoad(self) -> None:
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem.alloc().initWithImage_style_target_action_(
            UIImage.imageNamed_("barbut_options"), UIBarButtonItemStylePlain, self, SEL(b'onOptions')).autorelease()
    ...
    def refresh(self) -> None:
        ...
        if c:
            ...
            if not self.helper:
                self.helper = history.NewTxHistoryHelper(tv=self.tv, vc=self, noRefreshControl=True,
                                                         cls=history.TxHistoryHelperWithHeader, domain=c)
    ...
    def onOptions(self) -> None:
        ...
        show_contact_options_actionsheet(_Contact(self), self, self.navigationItem.rightBarButtonItem,
                                         navBackOnDelete=True, onEdit=onEdit)
    ...
    def onQRImgTap(self) -> None:
        if not self.qr.image:
            gui.ElectrumGui.gui.show_error(vc=self, message="Error, No QR Image")
        else:
            def ShowIt() -> None:
                utils.show_share_actions(vc=self, img=self.qr.image,
                                         ipadAnchor=self.qr.convertRect_toView_(self.qr.bounds, self.view),
                                         objectName=_("Image"))
...
def Find(addy) -> ContactsEntry:
    if not is_address(addy):
        return
    contacts = _Get()
    if addy and contacts:
        for c in contacts:
            if c.address == addy:
                return c
    return None
...
def build_contact_tx_list(address: str) -> list:
    parent = gui.ElectrumGui.gui
    ret = list()
    if address and parent and parent.sigHistory:
...
def get_contacts() -> list:
    ...
    for addr, tupl in c.items():
        typ, name = tupl
        if typ == 'address' and is_address(addr):
            address = addr
...
def pay_to(addys: list) -> bool:
    ...
    if len(addys) > 1:
        gui.ElectrumGui.gui.show_error(title=_("Coming Soon"),
                                       message=_(
                                           "This version of Electrum currently only supports sending to 1 address at a time! Sorry!"))
        return False
...
def show_new_edit_contact(contact, parentvc, onEdit=None, title=None) -> ObjCInstance:
    ...
    if contact:
        if isinstance(contact, ContactsEntry):
            pass
        if isinstance(contact, (tuple, list)) and len(contact) >= 2 and isinstance(contact[1], str) and isinstance(
                contact[0], str):
            contact = ContactsEntry(contact[0], contact[1], contact[1], list())
        elif isinstance(contact, str):
            contact = ContactsEntry('', contact, contact, list())
        else:
            raise ValueError(
                'First parameter to show_new_edit_contact must be either a ContactsEntry, a tuple, or an Address!')
...
def show_contact_options_actionsheet(contact: ContactsEntry, vc: ObjCInstance, view: ObjCInstance,
                                     navBackOnDelete=False, onEdit=None) -> None:
    ...
    try:
        ...
        utils.NSLog("*** WARNING: Exception during contacts.py 'show_contact_options_actionsheet': %s",
                    str(sys.exc_info()[1]))
