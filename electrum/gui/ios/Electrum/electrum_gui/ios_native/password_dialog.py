...
def Create_PWChangeVC(msg: str, hasPW: bool, isEnc: bool, hasTouchId: bool,
                      callback: Callable[[str, str, bool, bool], None],
                      # pass a callback that accepts oldPW, newPW, encrypt_wallet_bool
                      callbackChkTouchId: Callable[[bool], bool]
                      # pass a callback that gets invoked whenever they try to enable touchid. if it can't be done, returns false. arg to callable is whether we want to use or not use touchid (UISwitch isOn())
                      ) -> ObjCInstance:
...
class PWChangeVC(UIViewController):
    ...
    @objc_classmethod
    def pwChangeVCWithMessage_hasPW_isEncrypted_hasTouchId_(cls: ObjCInstance, msg: ObjCInstance, hasPW: bool,
                                                            isEnc: bool, hasTouchId: bool) -> ObjCInstance:
    ...
    def loadView(self) -> None:
        ...
        if msg is None:
            if not has_pw:
                ...
                if not is_encrypted:
                    msg = _('Your Noir are password protected. However, your wallet file is not encrypted.')
                else:
        ...
        self.colors = {"Weak": utils.uicolor_custom('red'), "Medium": UIColor.blueColor, "Strong": myGreen,
                       "Very Strong": myGreen}
...
def prompt_password_asynch(vc: ObjCInstance, onOk: Callable, prompt: str = None, title: str = None,
