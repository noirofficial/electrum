...
# Copyright (C) 2012-2018 Thomas Voegtlin <thomasv@electrum.org>
...
from . import wallets
from . import \
    newwallet  # Do not remove -- needed to declare NewWalletVC to ObjC runtime  (used by storyboard instantiation)
from .custom_objc import *

from electrum.i18n import _, set_language, languages
from electrum.plugin import run_hook
from electrum import WalletStorage, Wallet
from electrum.util import UserCancelled, print_error, format_satoshis, format_satoshis_plain, PrintError, \
    InvalidPassword, block_explorer_URL
...
class GuiHelper(NSObject):
    ...
    def bindRefreshControl_(self, refreshControl: ObjCInstance) -> None:
        if (refreshControl is not None
            ...
            refreshControl.addTarget_action_forControlEvents_(self, SEL('onRefreshControl:'),
    ...
    @objc_method
    def tabBarController_didEndCustomizingViewControllers_changed_(self, tabController, viewControllers,
                                                                   changed: bool) -> None:
        return
...
class ElectrumGui(PrintError):
    ...
    def __init__(self, config):
        ...
        self.appName = 'Electrum'
        self.appDomain = 'org.electrum.Electrum'
        self.set_language()
    ...
    def createAndShowUI(self):
        ...
        self.addressesVC = adr = addresses.AddressesVC.alloc().initWithMode_(UITableViewStylePlain,
                                                                             addresses.ModeNormal).autorelease()
    ...
    def on_backgrounded(self):
        ...
        for a in self.killableAlerts:
            if a.presentingViewController:
                ...
                a.dismissViewControllerAnimated_completion_(False,
                                                            Block(lambda: cb(), restype=None) if callable(cb) else None)
                ct += 1
    ...
    def on_status_update(self):
        ...
        if self.daemon.network is None or not self.daemon.network.is_connected():
            ...
            if not self.wallet.up_to_date or sh == 0:
                ...
                if self.daemon.fx.is_enabled():
                    text += self.daemon.fx.get_fiat_status_text(c + u + x, self.base_unit(),
                                                                self.get_decimal_point()) or ''
        ...
        if show_dl_pct is not None and self.tabController.didLayout:
            self.show_downloading_notif(
                _("Downloading headers {}% ...").format(show_dl_pct) if show_dl_pct > 0 else None)
        else:
            self.dismiss_downloading_notif()
    ...
    def query_hide_downloading_notif(self):
        ...
        utils.show_alert(vc=vc,
                         ...
                         message=_(
                             "Do you wish to hide the download progress banner?\n(You can re-enabled it in Settings later)"),
                         actions=[
    ...
    def add_navigation_bar_close_to_modal_vc(self, vc: ObjCInstance, leftSide=True, useXIcon=True) -> ObjCInstance:
        if useXIcon:
            ...
            closeButton = UIBarButtonItem.alloc().initWithTitle_style_target_action_(_('Cancel'),
    ...
    def format_amount(self, x, is_diff=False, whitespaces=False):
        return format_satoshis(x, is_diff=is_diff, num_zeros=self.num_zeros, decimal_point=self.decimal_point,
    ...
    def get_fee_estimation(self):
        choices = [_('Static'), _('ETA'), _('Mempool')]
        current_index = (2 if self.config.use_mempool_fees() else 1) if self.config.is_dynfee() else 0
        return choices, current_index

    def prefs_set_fee_estimation(self, index: int):
        self.config.set_key('mempool_fees', index == 2)
        self.config.set_key('dynamic_fees', index > 0)
    ...
    def prefs_get_use_rbf(self) -> bool:
        return self.config.get('use_rbf', True)

    def prefs_set_use_rbf(self, value):
        self.config.set('use_rbf', value)
    ...
    def prefs_get_use_change(
            self) -> tuple:  # returns the setting plus a second bool that indicates whether this setting can be modified
    ...
    def view_on_block_explorer(self, item: str, which: str) -> None:
        ...
        url = block_explorer_URL(self.config, which, item)
    ...
    def question(self, message, title=_("Question"), yesno=False, onOk=None, vc=None, destructive=False,
                 okButTitle=None,
                 onCancel=None) -> bool:
        ret = None
        ...
        if localRunLoop:
            utils.NSLog(
                "*** WARNING: It's been shown that local run loops are buggy on iOS. You are using one with an ElectrumGui.question() call.  Please fix this.")
        ...
        self.show_message(message=message, title=title, onOk=okFun, localRunLoop=localRunLoop, hasCancel=True,
                          **extrakwargs, vc=vc, destructive=destructive, onCancel=onCancel)
        return ret
    ...
    def delete_payment_request(self, addr: str, refreshDelay: float = -1.0) -> bool:
    ...
    def sign_payment_request(self, addr: str, onSuccess: Callable[[], None] = None,
                             onFailure: Callable[[], None] = None, vc: ObjCInstance = None):
        ...
        if alias and self.alias_info:
            ...
            if alias_addr:
                if self.wallet.is_mine(alias_addr):
                    ...
                    self.prompt_password_if_needed_asynch(callBack=DoSign, prompt=msg, vc=vc, onCancel=onFailure,
                                                          onForcedDismissal=onFailure)
        else:
            if callable(onSuccess): onSuccess()
    ...
    def pay_to_URI(self, URI, showErr: bool = True) -> bool:
        ...
        if r or (name and sig):
            ...
            if showErr: self.show_error(
                "Don't know how to handle this payment request type. Sorry!\n\nEmail the developers!")
            return False
    ...
    def warn_user_if_no_wallet(self) -> bool:
        if not self.wallet:
            ...
            self.show_message(title=_("No Wallet Is Open"),
                              message=_("To proceed, please select a wallet to open.") + " " + _(
                                  "You can also create a new wallet by selecting the 'Add new wallet' option."),
                              onOk=showDrawer)
            return True
        return False
    ...
    def stop_daemon(self):
        ...
        if self.tabController:
            ...
            if pres and not pres.isBeingDismissed(): self.tabController.dismissViewControllerAnimated_completion_(False,
                                                                                                                  None)
    ...
    def generate_new_wallet(self, wallet_name: str,
                            ...
                            have_keystore: object = None,
                            # not required but if we already have generated a keystore, will use this and ignore wallet_seed param
                            private_keys: list = [],
                            # if specified, ignores wallet_seed and have_keystore, do not specify this and watching_addresses at the same time
                            watching_addresses: list = [],
                            # if specified, ignores wallet_seed and have_keystore and wallet_pass.. do not specify this and private_keys at the same time
        ...
        if watching_addresses and (wallet_seed or have_keystore or wallet_pass or private_keys):
            raise ValueError(
                'Cannot specify watching_addresses along with other seed related params, private keys, or a wallet password')
            ...
            try:
                ...
                if private_keys:
                    for key in private_keys:
                        wallet.import_private_key(key, wallet_pass)
                elif watching_addresses:
                    for addr in watching_addresses:
    ...
    def switch_wallets(self, wallet_name: str,
                       onSuccess=None,
                       # cb signature is fun(), and this indicates the new wallet is now opened successfully
            ...
            def promptPW() -> None:
                ...
                def onOk(password: str) -> None:
                    try:
                        ...
                        self.show_error(
                            message=_("The password was incorrect for this encrypted wallet, please try again."),
                ...
                if wallet_pass:
                    ...
                    self.prompt_password_if_needed_asynch(callBack=onOk, prompt=prompt, title=title,
                                                          onCancel=myOnCancel,
                                                          onForcedDismissal=myOnCancel, usingStorage=storage,
                                                          vc=waitDlg)

            def promptPwLater() -> None:
                utils.call_later(0.1, promptPW)

            waitDlg = utils.show_please_wait(vc=vc, message="Opening " + wallet_name[:25] + "...",
                                             completion=promptPwLater)
        else:
            DoSwicheroo()

    def show_wallet_share_actions(self, info: wallets.WalletsMgr.Info, vc: UIViewController = None,
                                  ipadAnchor: object = None, warnIfUnsafe: bool = True) -> None:
                ...
                if not storage.is_encrypted():
                    ...
                    if not w.is_watching_only() and not w.has_password():
                        self.question(title=_("Potentially Unsafe Operation"),
                                      message=_(
                                          "This spending wallet is not encrypted and not password protected. Sharing it over the internet could result in others intercepting the data and for you to potentially lose funds.\n\nContinue anyway?"),
                                      yesno=True, vc=vc,
                                      onOk=lambda: self.show_wallet_share_actions(info=info, vc=vc,
        ...
        def DoIt() -> None:
            try:
                ...
                if fn:
                    ...
                    utils.show_share_actions(vc=waitDlg, fileName=fn, ipadAnchor=ipadAnchor,
                                             objectName=_('Wallet file'),
                                             finishedCompletion=lambda x: Dismiss())
                else:
                    def MyCompl() -> None:
                        self.show_error("Could not copy wallet file", vc=vc)
    ...
    def do_wallet_rename(self, info: wallets.WalletsMgr.Info, newName: str, vc: UIViewController = None,
                         password: str = None):
        ...
        if isCurrent and self.wallet.storage.is_encrypted() and not password:
            ...
            self.prompt_password_if_needed_asynch(callBack=gotPW, vc=vc,
                                                  prompt=_(
                                                      "You are renaming the currently open encrypted wallet '{}'.").format(
                                                      info.name))
            return
        ...
        try:
            ...
            if oldEncPw:
                self.encPasswords.set(newName, oldEncPw,
                                      save=False)  # migrate encrypted password to new name if present
    ...
    def prompt_password_if_needed_asynch(self, callBack, prompt=None, title=None, vc=None, onCancel=None,
                                         onForcedDismissal=None,
        ...
        def GotAPasswordCallback(password: str) -> None:
            ...
            if not touchIdTry:
                self.prompt_user_maybe_wants_touchid(wallet_name=wallet_name, password=password,
                                                     completion=CallUserCallback, vc=vc)
            else:
        ...
        def DoPromptPW(my_callback) -> ObjCInstance:
            ...
            if not touchIdTry and encpw:
                ...
                return password_dialog.prompt_password_asynch(vc=vc, onOk=my_callback, prompt=prompt, title=title,
                                                              onCancel=onCancel, onForcedDismissal=onForcedDismissal)

        if usingStorage:
            storage = WalletStorage(usingStorage, manual_upgrades=True) if not isinstance(usingStorage,
                                                                                          WalletStorage) else usingStorage
                    ...
                    def myCallback() -> None:
                        ...
                        if self.sendVC and not self.sendVC.isBeingDismissed():
                            ...
                            if self.sendVC.presentingViewController and not self.sendVC.presentedViewController and self.sendNav and self.sendNav.topViewController.ptr.value == self.sendVC.ptr.value:
                                self.sendVC.presentingViewController.dismissViewControllerAnimated_completion_(True,
                                                                                                               None)
    ...
    def change_password(self, oldpw: str, newpw: str, enc: bool, touchid: bool = False) -> None:
        ...
        try:
            ...
            return
        msg = _('Password was updated successfully') if newpw else _(
        ...
        if self.keyEnclave.has_keys():
            if not self.keyEnclave.biometrics_available():
                self.show_error(title=_("Biometrics Unavailable"),
                                message=_(
                                    "We will flag this wallet as requesting to use Touch/Face ID, however it is currently disabled on the device. Please enable Touch/Face ID from the iOS General settings in order to use biometrics for this wallet."))
            return True
        else:
            self.show_error(title=_("Secure Enclave Unavailable"),
                            message=_(
                                "This device lacks the secure enclave service. As such, Touch/Face ID based wallet authentication is unavailable."))
        return False
    ...
    def show_change_password(self, msg=None, vc=None):
        ...
        pwvc = password_dialog.Create_PWChangeVC(msg, self.wallet.has_password(),
                                                 ...
                                                 (self.encPasswords.has(
                                                     wallets.CurrentWalletName()) and self.is_touchid_possible()),
                                                 self.change_password,
                                                 self.check_touchid_for_gui)
        (self.get_presented_viewcontroller() if not vc else vc).presentViewController_animated_completion_(pwvc, True,
                                                                                                           None)
    ...
    def show_seed_dialog(self, password, vc=None):
        ...
        (vc if vc else self.get_presented_viewcontroller()).presentViewController_animated_completion_(seedvc, True,
                                                                                                       None)
    ...
    def show_network_dialog(self, vc=None) -> None:
        ...
        self.networkNav = utils.tintify(
            UINavigationController.alloc().initWithRootViewController_(self.networkVC).autorelease())
        ...
        (vc if vc else self.get_presented_viewcontroller()).presentViewController_animated_completion_(self.networkNav,
                                                                                                       True, None)

    def send_create_if_none(self) -> None:
        if self.sendVC: return
        self.sendVC = send.SendVC.alloc().init().autorelease()
        self.sendNav = utils.tintify(
            UINavigationController.alloc().initWithRootViewController_(self.sendVC).autorelease())
    ...
    def receive_create_if_none(self) -> None:
        ...
        self.receiveNav = utils.tintify(
            UINavigationController.alloc().initWithRootViewController_(self.receiveVC).autorelease())
    ...
    def show_ext_txn(self, txn: str) -> bool:
        ...
        try:
            if not self.wallet:
                self.show_error(_(
                    "Cannot display the requested transaction as you don't have a wallet open.\n\nOpen a wallet and try again."))
                return False
            ...
            if self.has_modal():
                self.show_error(_(
                    "Cannot display the requested transaction since you already have a modal dialog open.\n\nClose the current dialog and try again."))
            else:
                vc = self.get_presented_viewcontroller()
    ...
    def set_wallet_use_touchid(self, wallet_name: str, wallet_pass_or_none: str,
    ...
    def get_wallet_password_using_touchid(self, wallet_name: str, completion: Callable[[str], None],
    ...
    # this method is called by Electrum libs to start the GUI
    def main(self):
