    ...
    def loadView(self) -> None:
        ...
        for v in views:
            ...
            if isinstance(v, UIButton) and tag in BUTTON_TAGS:
                ...
                v.delegate = self
                if tag is TAG_HOST_TF:
                    self.hostTF = v
                elif tag is TAG_PORT_TF:
                    self.portTF = v
            ...
            if tag == TAG_SERVER_LBL:
                self.serverLbl = v
            elif tag == TAG_STATUS_LBL:
                self.statusLbl = v
            elif tag == TAG_HEIGHT_LBL:
                self.heightLbl = v
...
#        utils.nspy_put_byname(self, connected, 'connected')
    ...
    def tableView_cellForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        ...
        if tv.ptr == self.connectedTV.ptr:
            cell = tv.dequeueReusableCellWithIdentifier_(
                identifier)  # will always return a valid cell because we registered our nib in loadView
            ...
            if cdata is not None and len(cdata.items) > indexPath.row:
                ...
                l1.text = _("Unknown")
                l2.text = _("Unknown")
            cell.contentView.backgroundColor = UIColor.clearColor if not indexPath.row % 2 else UIColor.colorWithRed_green_blue_alpha_(
                0.0, 0.0, 0.0, 0.03)
        elif tv.ptr == self.peersTV.ptr:
            cell = tv.dequeueReusableCellWithIdentifier_(
                identifier)  # will always return a valid cell because we registered our nib in loadView
