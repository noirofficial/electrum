...
from electrum.util import timestamp_to_datetime, NotEnoughFunds
...
class SendVC(SendBase):
    ...
    def reader_didScanResult_(self, reader, result) -> None:
        ...
        if self.qrScanErr:
            if type(self.qrScanErr) is int and self.qrScanErr == 2:
                ...
                message = _(
                    "The QR code contains multiple outputs. At this time only a single output is supported.\nPlease try again.")
            else:
    ...
    def loadView(self) -> None:
            ...
            if fx() and fx().is_enabled():
                ...
                if rate:
                    amtfiat = int(round(float((Decimal(self.amountSats) * Decimal(100.0) * Decimal(rate)) / Decimal(
                        1e8)))) if self.amountSats is not None else None
        ...
        def onAmountFiat(t: ObjCInstance) -> None:
            ...
            if not rate: return
            amtSats = int(round(
                float((Decimal(t.getAmount()) * Decimal(1e6)) / Decimal(rate)))) if t.getAmount() is not None else None
        ...
        def sliderCB(dyn: bool, pos: int, fee_rate: int) -> None:
            txt = " ".join(str(slider.getToolTip(pos)).split("\n"))
            feelbl.text = txt
    ...
    def viewWillAppear_(self, animated: bool) -> None:
        ...
        # fee amount label
        lbl = self.feeLbl
        lbl.text = self.feeSlider.getToolTip(-1).replace("\n", " ")
    ...
    def viewDidAppear_(self, animated: bool) -> None:
        ...
        parent().show_warning_if_watching_only(vc=self,
                                               onOk=lambda: self.presentingViewController.dismissViewControllerAnimated_completion_(
                                                   True, None))
    ...
    def textFieldShouldEndEditing_(self, tf: ObjCInstance) -> bool:
        ...
        if tf.tag in [
            115]:  # the other ones auto-call chkOk anyway.. todo: make addr line edit be a special validating class
            self.chkOk()
    ...
    def checkQRData_(self, text) -> None:
        ...
        if len(lines) == 1:
            ...
            if payto_address and len(payto_address) and payto_address[1]:
        ...
        if len(errors):
            ...
            self.isMax = is_max
            self.onPayTo_message_amount_(outputs[0][1], "", outputs[0][2])
            self.updateFee()  # schedule a fee update later
        utils.NSLog("onCheckPayToText_ result: is_max=%s outputs=%s total=%s errors=%s", str(is_max), str(outputs),
                    str(total), str(errors))

    @objc_method
    def updateFee(self):
        # Enqueues a doUpdateFee() call for later -- this facility is provided for the fee slider so that it doesn't behave too slowly.
        def onTimer() -> None:
            self.timer = None
            self.doUpdateFee()
        ...
        if amount is None:
            ...
            try:
                ...
                if tx and freeze_fee and fee and tx.estimated_size():
                    self.feeLbl.text = _("Manual fee") + ": " + parent().format_fee_rate(
    ...
    def showTransaction_desc_(self, txraw, desc) -> None:
        ...
        tx.deserialize()
        tx_hash, status_, label_, can_broadcast, can_bump, amount, fee, height, conf, timestamp, exp_n = wallet().get_tx_info(tx)
        ...
        entry = HistoryEntry(tx, tx_hash, status_str, str(desc), self.amt.text, "",
                             timestamp_to_datetime(time.time() if conf <= 0 else timestamp), timestamp, conf, status,
                             amount, None, None, fiat_amount_str, None, ccy, None)
    ...
    def tableView_cellForRowAtIndexPath_(self, tableView, indexPath):
        ...
        try:
            ...
            cell.address.text = coin['address']
    ...
    def tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        ...
        try:
            ...
            utils.NSLog("Send.tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_, got exception: %s",
                        str(sys.exc_info()[1]))
            traceback.print_exc(file=sys.stderr)
        return None
...
def read_send_form(send: ObjCInstance) -> tuple:
    ...
    if False:  # self.payment_request:
        ...
        try:
            ...
            return None
        outputs = [TxOutput(typ, addr, "!" if send.isMax else amt_e.getAmount())]
