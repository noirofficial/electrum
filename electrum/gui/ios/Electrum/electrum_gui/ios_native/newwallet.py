...
from . import utils
from . import gui
from electrum.i18n import _, language
...
import electrum.keystore as keystore


#################################################################################################
#    Use the below 2 functions to call up either the On-Boarding or the "New Wallet" wizards    #
#################################################################################################
def PresentAddWalletWizard(vc: ObjCInstance = None, animated: bool = True, completion: Block = None,
                           dontPresentJustReturnIt=False) -> ObjCInstance:
...
def PresentOnBoardingWizard(vc: ObjCInstance = None, animated: bool = True, completion: Block = None,
                            dontPresentJustReturnIt=False) -> ObjCInstance:
    if not vc: vc = gui.ElectrumGui.gui.get_presented_viewcontroller()
    sb = UIStoryboard.storyboardWithName_bundle_("NewWallet", None)
    if not sb:
...
class NewWalletVC(NewWalletVCBase):
    ...
    def setupNextButtonSmartLayoutMogrificationWhenKeyboardIsShown(self) -> None:
        ...
        self.origPS = self.errMsg.attributedText.attribute_atIndex_effectiveRange_(NSParagraphStyleAttributeName, 0,
                                                                                   None)
        ...
        def slideUpButton(rect: CGRect) -> None:
            ...
            if utils.is_ipad():
                ...
                ats = NSMutableAttributedString.alloc().initWithAttributedString_(
                    self.errMsg.attributedText).autorelease()
    ...
    def translateUI(self) -> None:
        ...
        self.touchIdTit.setText_withKerning_(_("Use Touch/Face ID"), utils._kern)

        for tf, text in [
                (self.walletName, _("My Wallet")),
                (self.walletPw1, _("Tap to enter a password")),
                (self.walletPw2, _("Enter the same password again"))]:
            tf.placeholder = text
    ...
    def doChkFormOk(self) -> bool:
        ...
        if not self.walletName.text:
            ...
            errMsg = _(
                "Wallet password is empty. Please set a wallet password to proceed. You can disable wallet password protection later if you wish.")
        elif not self.noPWCheck and self.walletPw1.text != self.walletPw2.text:
            errMsg = _(
                "Wallet passwords do not match. Please confirm the password you wish to set for your wallet by entering the same password twice.")
...
class NewWalletSeed1(NewWalletSeedBase):
    ...
    def viewDidLoad(self) -> None:
        send_super(__class__, self, 'viewDidLoad')
        utils.uilabel_replace_attributed_text(self.seedtv, " ",
                                              font=UIFont.systemFontOfSize_weight_(16.0, UIFontWeightBold))

    @objc_method
    def translateUI(self) -> None:
        utils.uilabel_replace_attributed_text(self.info, _("Please write your seed phrase down, as it's the only way "
                                                           "to recover your funds if you forget your password or your"
                                                           " device is stolen."),
    ...
    def viewDidAppear_(self, animated: bool) -> None:
        ...
        if not self.seed:
            ...
            seed_type = _Params(self)['WalletType']

            def GenSeed() -> str:
                return _Mnem().make_seed(seed_type=seed_type)
...
class NewWalletSeed2(NewWalletSeedBase):
    ...
    @objc_method
    def translateUI(self) -> None:
        self.seedTit.setText_withKerning_(_("Enter your seed phrase"), utils._kern)
        if self.seedExtTit:
            self.seedExtTit.setText_withKerning_(_("Seed Extension"), utils._kern)
        if self.bip39Tit:
            self.bip39Tit.setText_withKerning_(_("BIP39 Seed"), utils._kern)
        if not self.restoreMode:
            txt = _('Your seed is important!') + ' ' + _(
                'To make sure that you have properly saved your seed, please retype it here.') + ' ' + _(
                'Use the quick suggestions to save time.')
        else:
            txt = _('You can restore a wallet that was created by any version of Electrum.')
        utils.uilabel_replace_attributed_text(self.info, txt, font=UIFont.italicSystemFontOfSize_(14.0))
        ...
        if len(suggestions) < 10:
            ...
            for sug in sugSet:
                def AddButWord(but: objc_id) -> None:
                    ...
                    try:
                        self.seedtv.setText_(
                            (' '.join(words[:wordNum]) + (' ' if wordNum else '') + word + ' ').lower())
                    except:
    ...
    def viewWillTransitionToSize_withTransitionCoordinator_(self, size: CGSize, coordinator: ObjCInstance) -> None:
        send_super(__class__, self, 'viewWillTransitionToSize:withTransitionCoordinator:', size, coordinator,
                   argtypes=[CGSize, objc_id])
    ...
    def onNext(self) -> None:
        ...
        if list(self.seedList) != self.seedtv.text.strip().lower().split():
            err = _(
                'The seed you entered does not match the generated seed. Go back to the previous screen and double-check it, then try again.')
...
class RestoreWallet1(NewWalletSeed2):
    ...
    def onNext(self) -> None:
        ...
        def ToErrIsHuman(title="Oops!", message="Something went wrong! Please email the developers!",
                         onOk=None) -> None:
        ...
        if seed_type == 'bip39':
            ...
            def onOk(text: str) -> None:
                ...
                if not derOk:
                    ToErrIsHuman(title=_('Derivation Invalid'),
                                 message=_('It appears the derivation you specified is invalid. Please try again'),
                                 onOk=lambda: self.onNext())
                    return
                if self.doBip44Keystore(seed, seedext, der):
                    PushIt()
                else:
                    ToErrIsHuman(title="else1")  # NB: we may already have an alert up from called code above, in which case this is a no-op (hacky but works!)
        ...
        elif seed_type in ['old', 'standard', 'segwit']:
            utils.NSLog("{} seed type".format(seed_type))
            if self.doStandardKeystore(seed, ''):
                PushIt()
            else:
                ToErrIsHuman(title="else2")  # NB: we may already have an alert up from called code above, in which case this is a no-op (hacky but works!)
        else:
            ToErrIsHuman(title="else4")
            return
...
class NewWalletMenu(NewWalletMenuBase):
    ...
    def viewWillAppear_(self, animated: bool) -> None:
        send_super(__class__, self, 'viewWillAppear:', animated, argtypes=[c_bool])
        _SetParams(self,
                   dict())  # clear any params that may be present if they were forward then back again as they may pick a different path down the wizard tree
...
class Import1(Import1Base):
    ...
    def viewDidLoad(self) -> None:
        ...
        self.tvDel.paragraphStyle = self.tv.attributedText.attribute_atIndex_effectiveRange_(
            NSParagraphStyleAttributeName, 0, None)
    ...
    def onQRBut(self) -> None:
        ...
        if not QRCodeReader.isAvailable:
            ...
            self.qrvc = QRCodeReaderViewController.readerWithCancelButtonTitle_codeReader_startScanningAtLoad_showSwitchCameraButton_showTorchButton_(
                "Cancel", self.qr, True, False, False)
    ...
    def doChkFormOk(self) -> bool:
        ...
        if self.masterKeyMode:
            if len(words) > 1:
                ErrMsg(_(
                    "You appear to have entered more than one item. Each wallet can only have a single master key. To use multiple master keys, create a new wallet for each key."))
            elif not words:
                ErrMsg(_(
                    "Please specify a master key to continue. Valid keys are long strings starting with either xpub/ypub/zpub or xprv/yprv/zprv."))
            elif keystore.is_master_key(words[0]):
                ClearErrMsg()
                return True
            else:
                ErrMsg(_(
                    "The provided key does not appear to be a valid master key. Valid keys are long strings starting with either xpub/ypub/zpub or xprv/yprv/zprv. Please try again."))
        else:
            for w in words:
...
class Import2(Import2Base):
    ...
    def viewDidLoad(self) -> None:
        ...
        ats.addAttribute_value_range_(NSKernAttributeName, utils._kern,
                                      NSRange(0, ats.length()))  # make error msg be kerned a bit more to fit better
        self.errMsg.attributedText = ats
    ...
    def viewDidAppear_(self, animated: bool) -> None:
        ...
        if self.masterKeyMode and not self.items:
            ...
            if not k:
                ...
                _ToErrIsHuman(self, title="else5")
                return
            ...
            def Get20Addys() -> list():
                ...
                for i in range(0, 20):
                    # TODO
                    addr = bitcoin.pubkey_to_address("p2pkh", k.derive_pubkey(0, i))
                    ret.append(addr)
                return ret

            def Err(excinf):
                utils.NSLog(
                    "Something went wrong! Got an exception when deriving addresses in Import2.viewDidAppear_: %s",
                    str(excinf[1]))
                _ToErrIsHuman(self, title="else6")

            def Success(l: list) -> None:
                self.items = l
                self.refresh()

            utils.WaitingDialog(vc=self, message=_("Deriving addresses..."), task=Get20Addys, on_success=Success,
                                on_error=Err)
    ...
    def tableView_cellForRowAtIndexPath_(self, tableView, indexPath):
        ...
        try:
            ...
            if ii.typ in [ImportItem.ADDR_LEGACY, ImportItem.ADDR_SEGWIT]:
                t = _("Legacy Address") if ii.typ == ImportItem.ADDR_LEGACY else _("Segwit Address")
                cell.desc.text = t + (" " + _("(watching-only)") if k and k.is_watching_only() else "")
            elif ii.typ == ImportItem.PRIVATE_KEY:
                cell.desc.text = _("Private Key - Address: {}").format(ii.info)
            else:
                cell.desc.text = ""
            ...
            cell = UITableViewCell.alloc().initWithStyle_reuseIdentifier_(UITableViewCellStyleSubtitle,
                                                                          "ACell").autorelease()
            cell.textLabel.text = " "
        return cell
    ...
    def tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_(self, tv, indexPath) -> ObjCInstance:
        ...
        try:
            ...
            utils.NSLog("Impoert2.tableView_trailingSwipeActionsConfigurationForRowAtIndexPath_, got exception: %s",
                        str(sys.exc_info()[1]))
            traceback.print_exc(file=sys.stderr)
        return None
    ...
    def doChkFormOk(self) -> bool:
        ...
        if self.masterKeyMode:
            ...
            for x in items:
                ...
                if item.typ in [ImportItem.ADDR_LEGACY, ImportItem.ADDR_SEGWIT]:
                    numaddr += 1
                elif item.typ == ImportItem.PRIVATE_KEY:
                    numpk += 1
                if item.typ:
            ...
            if self.forceType == 1 and numpk:
                msg += " " + _(
                    "This is a wathching-only wallet, so the given private keys will be converted to watching addresses.")
                ...
                msg = _(
                    "Cannot specify private keys and addresses in the same wallet. Addresses will result in a watching-only wallet, and private keys in a spending wallet. Remove incompatible items (by swiping them left).")
                ret = False
                asError = True
            elif numpk:
    ...
    def onSave(self) -> None:
        ...
        if self.doChkFormOk():
            try:
                ...
                _ToErrIsHuman(vc=self, title="else8")


####################
# Useful Helpers   #
####################
ImportItem = namedtuple("ImportItem", "item typ info")
ImportItem.INVALID = 0
ImportItem.ADDR_LEGACY = 1
ImportItem.ADDR_SEGWIT = 2
ImportItem.PRIVATE_KEY = 3
...
def _ImportItemify(item: str) -> ImportItem:
    ...
    info = None
    typ = 0
    if bitcoin.is_address(item):
        typ = 2 if bitcoin.is_segwit_address(item) else 1
        info = item
        item = item.split(':')[-1]
...
def _ToErrIsHuman(vc: UIViewController, title="Oops!", message="Something went wrong! Please email the developers!",
                  onOk=None) -> None:
    ...
    def onSuccess() -> None:
        ...
        if not _IsOnBoarding(vc):
            parent.show_message(vc=vc, title=_('New Wallet Created'),
                                message=_(
                                    'Your new imported wallet has been successfully created. Would you like to switch to it now?'),
...
class OnBoardingWizard(OnBoardingWizardBase):
    ...
    def viewDidLoad(self) -> None:
        ...
        if self.pvc:
            ...
            if not vcs or None in vcs:
                utils.NSLog("ERROR: Could not find a requisite viewcontroller in %s viewDidLoag method!",
                            str(__class__))
                return
            ...
            self.pvc.setViewControllers_direction_animated_completion_(NSArray.arrayWithObject_(vcs[0]),
    ...
    def onNextButton_(self, curidx: int) -> None:
        ...
        if nextidx < len(vcs):
            ...
            self.pvc.setViewControllers_direction_animated_completion_(NSArray.arrayWithObject_(vcs[nextidx]),
                                                                       UIPageViewControllerNavigationDirectionForward,
                                                                       True, None)
...
class OnBoardingMenu(OnBoardingMenuBase):
    ...
    def jumpToMenu_(self, vcToPushIdentifier) -> None:
        ...
        if isinstance(vc, NewWalletNav) and vc.viewControllers and isinstance(vc.viewControllers[0], NewWalletMenu):
            ...
            sb = UIStoryboard.storyboardWithName_bundle_('NewWallet', None)
            if sb:
                vc2 = sb.instantiateViewControllerWithIdentifier_(
                    vcToPushIdentifier)  # NB: If you rename it in storyboard be SURE to update this!
                if vc2:
